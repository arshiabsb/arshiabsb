# Hi there, I'm Arshia! 👋

## 🚀 About Me
- 💻 **Data Automation Engineer:** Building Python pipelines that automate ledger balancing, zip file data extraction, and automated cross-checks.
- 🛠 **Quality Focused:** Writing testing suites to validate financial data models (handling zero and negative balances perfectly).
- ⚙️ **Process Optimizer:** Replacing manual spreadsheet accounting tasks with automated software logic.

---

## 🛠 Tech Stack & Tools

### 🐍 Languages & Frameworks
[![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)](https://www.python.org/)
[![Git](https://img.shields.io/badge/Git-F05033?style=for-the-badge&logo=git&logoColor=white)](https://git-scm.com/)

### 💻 IDEs & Environment
[![Cursor](https://img.shields.io/badge/Cursor-000000?style=for-the-badge&logo=cursor&logoColor=white)](https://cursor.com/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com)
[![Windows](https://img.shields.io/badge/Windows-0078D4?style=for-the-badge&logo=windows&logoColor=white)](https://www.microsoft.com/windows)

---

## 📊 Live Interactive Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=arshiabsb&show_icons=true&theme=tokyonight&count_private=true" width="48%"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=arshiabsb&theme=tokyonight" width="48%"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=arshiabsb&layout=compact&theme=tokyonight" width="60%"/>
</p>
---

## 📈 Current Focus & Coding Journey
- **Data Engineering:** Designing seamless pipeline workflows that convert raw `.zip` deliverables into production-ready ledger lines.
- **Defensive Programming:** Leveraging Python's `unittest` framework to catch data inconsistencies before they reach management.
